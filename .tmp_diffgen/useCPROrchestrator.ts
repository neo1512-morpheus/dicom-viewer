...
import { getDefaultCrossSectionVoiRange } from './crossSectionImageLoader';
...
const CPR_CROSSSECTION_DEFAULT_VOI_LUT_FUNCTION = 'LINEAR';
...
function isVolumeViewportLike(
  ...
  return (
    ...
    typeof candidate.getActors === 'function'
  );
}
...
function applyCrossSectionVolumeViewportPresentation(
  viewport: cornerstone.Types.IViewport | null,
  sourceVolumeId?: string | null,
  voiRange = getDefaultCrossSectionVoiRange()
): void {
  if (!viewport || !isVolumeViewportLike(viewport)) {
    return;
  }
  ...
  const properties = {
    voiRange,
    invert: false,
    VOILUTFunction: CPR_CROSSSECTION_DEFAULT_VOI_LUT_FUNCTION,
    slabThickness: CPR_CROSSSECTION_DEFAULT_SLAB_THICKNESS_MM,
  } as any;

  viewport.setBlendMode?.(CPR_CROSSSECTION_DEFAULT_BLEND_MODE, actorUIDs);
  if (sourceVolumeId) {
    viewport.setProperties?.(properties, sourceVolumeId);
  } else {
...
async function initializeCrossSection(
  ...
  console.log('[CPR-CROSSSECTION-CONFIG-JSON]', JSON.stringify({
    ...
    verticalCenterOffsetMm: toFiniteNumber(verticalCenterOffsetMm) ?? 0,
    geometryMode: 'native-volume-mpr',
  ...
  applyCrossSectionVolumeViewportPresentation(crossViewport, sourceVolumeId);
